...
void setup() 
{
  ...
  posYRastro = new float [40];
  posXRastro = new float [40];

  for (int i = 0; i < posYRastro.length; i++)
  {
    posYRastro[i] = -30;
    posXRastro[i] = -30;
  }
  ...
  //Desenha a bola
  imageMode(CENTER);
  image(bola, MRU(vHor0, t), distVert, 30, 30);
...
void draw() 
{
  ...
  fill(0,180,80);
  rect(0,550, width, 600);
  mostraGrid();

  //Calcula a distância do chão na vertical
  distVert = 600 + MRUV(vVert0, grav, t);

  //Atualiza os valores do rastro
  if (t >= delayRastro)
  {
    if (currentIndex < posYRastro.length)
    {
      posXRastro[currentIndex] = MRU(vHor0, t);
      posYRastro[currentIndex] = distVert;
      currentIndex++;
    }
    delayRastro += 60;
  }  
  ...
  //Desenha a bola
  imageMode(CENTER);
  image(bola, MRU(vHor0, t), distVert, 35, 35);
...
void mostraRastro()
{
  ...
  {
    fill(200, 75, 75);
    ellipse(posXRastro[i], posYRastro[i], 30, 30);
  }
}
